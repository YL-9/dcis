...
from transformers import set_seed, default_data_collator, get_linear_schedule_with_warmup, get_constant_schedule_with_warmup, AutoConfig, AutoModelForCausalLM
...
def main(args):
    ...
    accelerator.init_trackers(
        project_name=args.wandb if args.wandb else "dcis",
    )
    ...
    config = config_cls.from_pretrained(args.model, trust_remote_code=True)
    factors = torch.load(args.scaling_factors)
    print("factors:\n", factors)
    config.rope_scaling = {
    ...
    if args.truncate:
        ...
        train_dataset.save_to_disk(f"{args.dataset}-truncated-{args.truncate}")
        return
        ...
        for step, batch in enumerate(train_loader):
            ...
            if accelerator.sync_gradients:
                ...
                if isinstance(args.checkpointing_steps, int) and completed_steps > 0:
                   if completed_steps % args.checkpointing_steps == 0 and completed_steps != args.max_train_steps:
...
if __name__ == "__main__":
    ...
    args.add_argument("--model", type=str,
                      default="/data/supry/models/llama-2/llama2-7b-hf")
    args.add_argument("--scaling-factor", type=float, default=16.0)
    args.add_argument("--scaling-type", type=str, default="dcis")
    args.add_argument("--rope-theta", type=float, default=10000.0)
    args.add_argument("--truncate", type=int)
    args.add_argument("--dataset", type=str,
                      default="/data/yl-9/datasets/pg_books-tokenized-bos-eos-chunked-65536")
    ...
    args.add_argument("--architecture", type=str,
                      choices=["llama", "mistral", "fuxitranyu"], default="llama")
    ...
    args.add_argument("--scaling-factors", type=str)
